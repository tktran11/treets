...
__all__ = ['file_loader', 'find_date', 'find_float_time', 'week_from_start', 'load_public_data', 'in_good_logging_day',
           'convert_loggings', 'get_types', 'count_caloric_entries', 'mean_daily_eating_duration',
           'filtering_usable_data', 'prepare_baseline_and_intervention_usable_data', 'users_sorted_by_activity',
           'eating_intervals_percentile', 'summarize_data', 'breakfast_analysis_summary', 'dinner_analysis_summary',
           'breakfast_mean_with_error_bar', 'dinner_mean_with_error_bar', 'breakfast_analysis_variability_plot',
           'dinner_analysis_variability_plot', 'breakfast_avg_histplot', 'breakfast_sample_distplot',
           'dinner_avg_histplot', 'dinner_sample_distplot', 'swarmplot', 'FoodParser']
...
# Cell
def file_loader(in_path, pattern='*'):
    """
    Description:\n
        This is a helper function that 1. if in_path is a single file path in pickle/csv format, it reads it into a pd dataframe. 2. if it is a folder path, it reads csv files that match the pattern parameter in the in_path folder into one dataframe. \n
    ...
    if isinstance(in_path, str):
        ...
        elif len(in_path.split('.')) == 1:
            data_lst = glob.glob('{}/{}'.format(in_path,pattern))
            dfs = []
            for x in data_lst:
                dfs.append(pd.read_csv(x))
            df = pd.concat(dfs).reset_index(drop=True)
            print('read the csv files in {} folder successfully.'.format(in_path))
    else:
        df = in_path
...
def load_public_data(in_path, h):
    ...
    Requirements:\n
        in_path file must have the following columns:\n
            ...
            - unique_code\n
    """
    public_all = file_loader(in_path).drop(columns = ['foodimage_file_name'])
...
def convert_loggings(in_path, target_text, identifier):
    ...
    public_all = file_loader(in_path)
...
# Cell
def mean_daily_eating_duration(df, time_col):
    """
    Description:\n
    ...
    Requirement:\n
        ...
        - float time is calculated and saved as a column that will be passed in as the time_col parameter.
...
# Cell
def users_sorted_by_activity(in_path, food_type = ["f", "b", "m", "w"]):
    """
    Description:\n
...
# Cell
def summarize_data(in_path, identifier, float_time_col, date_col, min_log_num = 2, min_seperation = 4):
    """
    Description:\n
    ...
    breakfast_variability = breakfast_analysis_summary(df,identifier,date_col,float_time_col).set_index('id')
    for col in breakfast_variability.columns:
        if col == 'id' or col == '50%':
            continue
        breakfast_variability[col] = breakfast_variability[col] - breakfast_variability['50%']
    breakfast_ser = breakfast_variability['90%'] - breakfast_variability['10%']

    #dinner variation (90%-10%)
    dinner_variability = dinner_analysis_summary(df,identifier,date_col,float_time_col).set_index('id')
    for col in dinner_variability.columns:
        if col == 'id' or col == '50%':
            continue
        dinner_variability[col] = dinner_variability[col] - dinner_variability['50%']
...
# Cell
def breakfast_mean_with_error_bar(in_path, identifier, date_col, time_col, min_log_num=2, min_separation=4):
    """
    Description:\n
       This function takes the loggings in good logging days, calculates the means and standard deviations of breakfast time for each participant and represent the calculated data with a scatter plot where the x axis is participants and the y axis is hours in a day.\n
    ...
    breakfast_series = df.groupby([identifier, date_col])[time_col].min()


    # find means and stds for each person
    means = breakfast_series.groupby('unique_code').mean().to_frame().rename(columns={'local_time':'mean'})
    stds = breakfast_series.groupby('unique_code').std().fillna(0).to_frame().rename(columns={'local_time':'std'})
...
def dinner_mean_with_error_bar(in_path, identifier, date_col, time_col, min_log_num=2, min_separation=4):
    """
    Description:\n
       This function takes the loggings in good logging days, calculates the means and standard deviations of dinner time for each participant and represent the calculated data with a scatter plot where the x axis is participants and the y axis is hours in a day.\n
    ...
    dinner_series = df.groupby([identifier, date_col])[time_col].max()


    # find means and stds for each person
    means = dinner_series.groupby('unique_code').mean().to_frame().rename(columns={'local_time':'mean'})
    stds = dinner_series.groupby('unique_code').std().fillna(0).to_frame().rename(columns={'local_time':'std'})
...
# Cell
def breakfast_analysis_variability_plot(in_path,identifier, date_col, time_col, min_log_num=2, min_separation=4):
    """
    Description:\n
...
# Cell
def dinner_analysis_variability_plot(in_path, identifier, date_col, time_col, min_log_num=2, min_separation=4):
    """
    Description:\n
       This function calculates the variability of loggings in good logging day by subtracting 5%,10%,25%,50%,75%,90%,95% quantile of dinner time from the 50% dinner time and makes a histogram that represents the 90%-10% interval for all subjects.\n
