...
__all__ = ['read_logging_data', 'get_phase_duration', 'count_caloric_entries', 'mean_daily_eating_duration',
...
def find_percentiles(df, time_col, percentiles, start_date='not_defined', end_date='not_defined'):
    ...
    df = df[(df['date']>=start_date) & (df['date']<=end_date)]
    return df[time_col].describe(percentiles=percentiles)
...
def make_table(food_data, ref_tbl, report_level=2, min_log_num=2, min_seperation=5, buffer_time= '15 minutes', h=4):
    ...
    df['date'] = treets.find_date(df, 'original_logtime', h)
    df['float_time'] = treets.find_float_time(df, 'original_logtime', h)
